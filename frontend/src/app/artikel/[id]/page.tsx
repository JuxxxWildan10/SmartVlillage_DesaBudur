...
export default function BeritaDetailPage() {
  ...
  useEffect(() => {
    if (!params.id) return;
    axios.get(`${process.env.NEXT_PUBLIC_API_URL}/berita/${params.id}`)
      .then(res => {
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: berita?.judul,
        text: berita?.isi_berita?.substring(0, 100) + "...",
          ...
          <div className="px-8 py-5 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-6 text-sm text-gray-500">
              ...
              <span className="flex items-center gap-2">
                ...
                {new Date(berita.created_at).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })} WIB
              </span>
              <span className="flex items-center gap-2">
